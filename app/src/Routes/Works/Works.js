...
const Piece = (piece) => {
    ...
    return (
        <li key={key}>
            <div>{piece.title}</div>
            <div>{piece.type}</div>
            {instruments ? (<div>{instruments}</div>) : null}
...
const WorksCategory = (categoryName, category) => {
    const pieces = category ? category.map(c => Piece(c)) : null
    return (
        <li className="category" key={categoryName}>
            <div>{categoryName}</div>
            {pieces ? (<ul>{pieces}</ul>) : null}
        </li>
    );
}
...
export const Works = () => {
    ...
    const categories = categoryKeys ? categoryKeys.map(category => WorksCategory(category, works.categories[category])) : null
    return (
        <div>
            <Typography variant="h6">Works</Typography>
            {categories ? (<ul>{categories}</ul>): null}
        </div>
    );
}
